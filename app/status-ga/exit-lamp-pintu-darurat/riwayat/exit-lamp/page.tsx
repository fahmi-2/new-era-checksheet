...
const loadRecords = async () => {
  try {
    ...
    console.log('📦 Raw API data:', data.slice(0, 1));
    ...
    const formattedData = data.map((record: any, recordIdx: number) => {
      ...
      return {
        ...
        items: record.items.map((item: any, itemIdx: number) => ({
          ...
          lokasi: item.lokasi,
          id: item.id,
          kondisiLampu: item.kondisiLampu || 'OK',
          indikatorLampu: item.indikatorLampu || 'OK',
          kebersihan: item.kebersihan || 'OK',
    ...
    console.log('✅ Formatted data:', formattedData.slice(0, 1));
   ...
   // Load data dari API - Initial Load Only
useEffect(() => {
  loadRecords();
}, []);
    ...
    const generateNextNo = (items: ExitLampItem[]) => {
      const maxNo = Math.max(0, ...items.map(i => i.no || 0));
      return maxNo + 1;
    };

     const openEditModal = (record: ExitLampRecord) => {
  ...
  // ✅ VALIDASI: record.id harus ada dan bertype number
  ...
  setEditData({
    ...
    items: record.items.map((item) => {
      ...
      if (!item.itemId) {
        console.warn('⚠️ Item tidak punya itemId:', item);
      }
      return {
        ...item,
        itemId: item.itemId,  // ✅ Preserve dari API (tidak fallback)
        _action: 'update'
      };
    }),
    replaceItems: false
  ...
  setNewItemNo(generateNextNo(record.items) + 1);
      ...
      const handleItemChange = (index: number, field: keyof ExitLampItem, value: string) => {
        if (!editData) return;
                            ...
                            <div key={`${item.no}-${item.lokasi}-${index}`} className="edit-item-card">
                              ...
                              <div className="edit-item-fields">
                                ...
                                <div className="status-grid">
                                  {['kondisiLampu', 'indikatorLampu', 'kebersihan'].map((field) => (
                                    <div key={field} className="status-select-group">
                                      <label className="status-label">
                                        {field === 'kondisiLampu' ? 'Kondisi Lampu' : 
                                        field === 'indikatorLampu' ? 'Indikator' : 'Kebersihan'}
                                      </label>
                                      <select
                                        value={item[field as keyof ExitLampItem] || 'OK'}
                                        onChange={(e) => handleItemChange(realIndex, field as keyof ExitLampItem, e.target.value)}
                                        className={`status-select ${item[field as keyof ExitLampItem] === 'NG' ? 'ng' : 'ok'}`}
                                      >
